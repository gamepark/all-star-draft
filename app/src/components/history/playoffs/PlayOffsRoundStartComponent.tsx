...
export const PlayOffsRoundStartComponent: FC<MoveComponentProps<MaterialMove<PlayerColor, MaterialType, LocationType, RuleId>, PlayerColor>> = ({
  move,
  context
}) => {
  if (!isStartSimultaneousRule<PlayerColor, MaterialType, LocationType, RuleId>(move) || move.id !== RuleId.PlayoffSubstitutePlayers) {
    return null
  }
  ...
  return (
    <h4>
      <Trans defaults="history.playOffsPhase.playOffsRoundStart" values={{ roundNumber: roundNumber }} components={{ sup: <sup></sup> }} />
    </h4>
  )
}
