...
export class DraftRoundPhaseHeritageAndClashCardSelectionRule extends SimultaneousRule<PlayerColor, MaterialType, LocationType> {
  ...
  getMovesAfterPlayersDone(): MaterialMove<PlayerColor, MaterialType, LocationType>[] {
    if (this.material(MaterialType.HockeyPlayerCard).location(LocationType.HockeyPlayerDraftSpot).length > 0) {
      if (this.remind<RegularSeasonGameMode>(Memorize.GameMode) === RegularSeasonGameMode.Duel)
